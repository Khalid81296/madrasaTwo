...
class PaymentController extends Controller
{
    ...
    public function invoice($st_id, $year = NULL)
    {
        ...
        $singInv = $year ? $this->pay->getSingleAllMyPR($st_id, $year) : $this->pay->getSingleAllMyPR($st_id);
        ...
        $d['SingleUncleared'] = $spr->where('paid', 0);
        $d['SingleCleared'] = $spr->where('paid', 1);
        // return $d;
    ...
    public function singleReceipts($pr_id)
    {
        // return $pr_id;
        if(!$pr_id) {return Qs::goWithDanger();}

        try {
             $d['pr'] = $pr = $this->pay->getSingelRecord(['id' => $pr_id])->with('singleReceipt')->first();
        } catch (ModelNotFoundException $ex) {
            return back()->with('flash_danger', __('msg.rnf'));
        }
        $d['receipts'] = $pr->singleReceipt;
        $d['payment'] = $pr->singlePayment;
        $d['sr'] = $this->student->findByUserId($pr->student_id)->first();
        $d['s'] = Setting::all()->flatMap(function($s){
            return [$s->type => $s->description];
        });
    ...
    public function single_pay_now(Request $req, $pr_id)
    {
        $this->validate($req, [
            'amt_paid' => 'required|numeric'
        ], [], ['amt_paid' => 'Amount Paid']);

        $pr = $this->pay->findSingleRecord($pr_id);
        $payment = $this->pay->findSingle($pr->payment_id);
        $d['amt_paid'] = $amt_p = $pr->amt_paid + $req->amt_paid;
        $d['balance'] = $bal = $payment->amount - $amt_p;
        $d['paid'] = $bal < 1 ? 1 : 0;

        $this->pay->updateSingleRecord($pr_id, $d);

        $d2['amt_paid'] = $req->amt_paid;
        $d2['balance'] = $bal;
        $d2['single_payment_records_id'] = $pr_id;
        $d2['year'] = $this->year;

        $this->pay->createSingleReceipt($d2);
        return Qs::jsonUpdateOk();
    }
