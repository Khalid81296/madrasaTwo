...
class StudentRecordController extends Controller
{
   ...
   public function __construct(LocationRepo $loc, MyClassRepo $my_class, UserRepo $user, StudentRepo $student)
   {
    ...
    public function store(StudentRecordCreate $req)
    {
       ...
       $data =  $req->only(Qs::getUserRecord()); // Student User info
       $sr =  $req->only(Qs::getStudentData());//Student info
       ...
       if($req->hasFile('photo')) {
        ...
        for ($i=0; $i<sizeof($req->input('amount')); $i++) {
            if($req->amount[$i] != null){
                ...
                // $singlePay['student_id'] = $student->id;
                $singlePay['student_id'] = $user->id;
                ...
                $singlePayRecords['student_id'] = $user->id;
    ...
    public function destroy_old($st_id)
    {
        dd(1);
        $st_id = Qs::decodeHash($st_id);
    ...
    {       
        // dd($id);
        $data =DB::table('users')
                    ->leftJoin('student_records','users.id', '=','student_records.user_id')
                    ->where('users.id', $id); 
        DB::table('student_records')->where('user_id', $id)->delete();                           
        $data->delete();
        return back()->with('flash_success', __('msg.del_ok'));
