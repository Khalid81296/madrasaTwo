...
class MarkController extends Controller
{
    ...
    public function show($student_id, $year)
    {
        ...
        $d['exam_records_count'] = DB::table('exam_records')->where('student_id',$wh)->where('year',$year)->count();
        // return $d;
    ...
    public function final_marks_print_view($student_id, $exam_id, $year)
    {
        /* Prevent Other Students/Parents from viewing Result of others */
        if(Auth::user()->id != $student_id && !Qs::userIsTeamSA() && !Qs::userIsMyChild($student_id, Auth::user()->id)){
            return redirect(route('dashboard'))->with('pop_error', __('msg.denied'));
        ...
        //$d['mark_type'] = Qs::getMarkType($ct);
        // return $d;
        return view('pages.support_team.marks.print.finalMarksIndex', $d);
    }
    ...
    {
        // return $exam_id.$class_id.$section_id.$subject_id;
