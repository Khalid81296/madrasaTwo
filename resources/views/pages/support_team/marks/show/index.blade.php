...
<div class="card">
    <div class="card-header header-elements-inline">
        <h6 class="font-weight-bold">Final Result {{ $year }}</h6>
        {!! Qs::getPanelOptions() !!}
    </div>

    <div class="card-body collapse" style="display: block;">

        {{-- Sheet Table --}}
        @include('pages.support_team.marks.show.finalSheet')

        {{-- Print Button --}}
        <div class="text-center mt-3">
            <a target="_blank" href="{{ route('finalMarks.print', [Qs::hash($student_id), $ex->id, $year]) }}"
