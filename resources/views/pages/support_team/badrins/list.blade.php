...
@section('page_title', 'Badrin Information - '.$my_class->name)
@section('content')

    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title">Badrins List</h6>
            {!! Qs::getPanelOptions() !!}
        </div>

        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                <li class="nav-item"><a href="#all-badrins" class="nav-link active" data-toggle="tab">All {{ $my_class->name }} Badrins</a></li>
            ...
            <div class="tab-content">
                <div class="tab-pane fade show active" id="all-badrins">
                                    ...
                                    <div class="list-icons">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-left">
                                                <a href="{{ route('badrins.show', Qs::hash($s->id)) }}" class="dropdown-item"><i class="icon-eye"></i> View Profile</a>
                                                @if(Qs::userIsTeamSA())
                                                    <a href="{{ route('badrins.edit', Qs::hash($s->id)) }}" class="dropdown-item"><i class="icon-pencil"></i> Edit</a>
                                                ...
                                                @if(Qs::userIsSuperAdmin())
                                                    ...
                                                    <form method="post" id="item-delete-{{ Qs::hash($s->user->id) }}" action="{{ route('badrins.destroy', Qs::hash($s->user->id)) }}" class="hidden">@csrf @method('delete')</form>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                ...
                @foreach($sections as $se)
                    <div class="tab-pane fade" id="s{{$se->id}}">                         <table class="table datatable-button-html5-columns">
                            ...
                            <tbody>
                            @foreach($badrins->where('section_id', $se->id) as $sr)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a href="{{ route('badrins.show', Qs::hash($sr->id)) }}" class="dropdown-item"><i class="icon-eye"></i> View Info</a>
                                                    @if(Qs::userIsTeamSA())
                                                        <a href="{{ route('badrins.edit', Qs::hash($sr->id)) }}" class="dropdown-item"><i class="icon-pencil"></i> Edit</a>
                                                    ...
                                                    @if(Qs::userIsSuperAdmin())
                                                        ...
                                                        <form method="post" id="item-delete-{{ Qs::hash($sr->user->id) }}" action="{{ route('badrins.destroy', Qs::hash($sr->user->id)) }}" class="hidden">@csrf @method('delete')</form>
                                                    @endif
